...
async fn scan_folder(folder_path: String) -> Result<Vec<SongMetadata>, String> {
    ...
    for entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
        ...
        let ext = fp
            ...
            .unwrap_or_default();
        if !["mp3", "m4a", "flac", "wav", "ogg", "opus"].contains(&ext.as_str()) {
            continue;
        }
        ...
        if let Ok(tf) = tf {
            ...
            if let Some(tag) = tf.primary_tag().or_else(|| tf.first_tag()) {
                ...
                if let Some(v) = tag.genre() {
                    let r = resolve_id3_genre(v.trim());
                    if !r.is_empty() {
                        genre = r;
                    }
                }
        ...
        if let Some(tag) = tf.primary_tag().or_else(|| tf.first_tag()) {
            if let Some(pic) = tag.pictures().first() {
                ...
                return Ok(Some(format!(
                    "data:{};base64,{}",
                    mime,
                    general_purpose::STANDARD.encode(pic.data())
                )));
            }
        }
    }
...
async fn save_playlist(
    ...
    let file =
        std::fs::File::create(dir.join(format!("{}.json", safe))).map_err(|e| e.to_string())?;
        ...
        if let Ok(e) = entry {
            ...
            if p.is_file() && p.extension().map(|s| s == "json").unwrap_or(false) {
                if let Ok(f) = std::fs::File::open(&p) {
                    if let Ok(pl) = serde_json::from_reader::<_, Playlist>(f) {
                        out.push(pl);
                    }
                }
            }
...
async fn get_history(app_handle: tauri::AppHandle) -> Result<Vec<SongMetadata>, String> {
    ...
    serde_json::from_reader(std::fs::File::open(f).map_err(|e| e.to_string())?)
        .map_err(|e| e.to_string())
}
...
async fn save_history(
    ...
    serde_json::to_writer_pretty(
        std::fs::File::create(dir.join("history.json")).map_err(|e| e.to_string())?,
        &history,
    )
    .map_err(|e| e.to_string())
}
...
async fn save_favourites(
    ...
    std::fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
    serde_json::to_writer_pretty(
        std::fs::File::create(dir.join("favourites.json")).map_err(|e| e.to_string())?,
        &songs,
    )
    .map_err(|e| e.to_string())
}
...
async fn update_metadata(
    ...
    let tag = if tf.primary_tag().is_some() {
        tf.primary_tag_mut().unwrap()
    } else {
        tf.first_tag_mut().unwrap()
    };
    ...
    match publisher {
        Some(v) if !v.is_empty() => {
            tag.insert_text(ItemKey::Publisher, v);
        }
        _ => {
    ...
    match copyright {
        Some(v) if !v.is_empty() => {
            tag.insert_text(ItemKey::CopyrightMessage, v);
        }
        _ => {
    ...
    match isrc {
        Some(v) if !v.is_empty() => {
            tag.insert_text(ItemKey::Isrc, v);
        }
        _ => {
    ...
    tf.save_to_path(fp, lofty::config::WriteOptions::default())
        .map_err(|e| e.to_string())
}
...
fn url_decode(s: &str) -> String {
    ...
    while let Some(b) = chars.next() {
        if b == b'%' {
            ...
            if let Ok(p) = u8::from_str_radix(&format!("{}{}", h1, h2), 16) {
                bytes.push(p);
            }
        } else if b == b'+' {
            bytes.push(b' ');
    ...
    thread::spawn(move || {
        for stream in listener.incoming() {
            if let Ok(mut stream) = stream {
                thread::spawn(move || {
                    let mut reader =
                        BufReader::new(unsafe { &mut *(&mut stream as *mut std::net::TcpStream) });
                    ...
                    if url.starts_with("/cover") {
                        if let Some(i) = url.find("?path=") {
                            ...
                            if fp.exists() && fp.is_file() && serve_cover(&mut stream, fp) {
                                return;
                            }
                        }
                                ...
                                if let Ok(meta) = file.metadata() {
                                    ...
                                    if let Some(rng) = range_hdr {
                                        if let Some(bi) = rng.find("bytes=") {
                                            ...
                                            let end: u64 = if rp.len() > 1 && !rp[1].is_empty() {
                                                rp[1].parse().unwrap_or(sz - 1)
                                            } else {
                                                sz - 1
                                            };
                                            if start < sz {
